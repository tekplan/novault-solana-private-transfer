  ...
  return (
    <section id="os" className="relative py-32 md:py-40 border-t border-[var(--border)]">
      ...
      <div className="max-w-[1280px] mx-auto px-6 md:px-10 relative">
        <SectionHeader
          number="06"
          eyebrow="Private Transfer OS"
          align="split"
          title={<>A command center for agentic private movement.</>}
          body="A working interface — not a mockup. Let the agent prepare intents, generate proofs, move encrypted balances, and audit settlement, all while the wallet retains full custody."
        />
            ...
            <div className="grid grid-cols-12 gap-3 relative">
              ...
              <aside className="col-span-12 md:col-span-2 glass rounded-2xl p-4">
                ...
                <nav className="space-y-1">
                  {sidebarItems.map(([Icon, label, active], i) => (
                    <div
                      key={i}
                      className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] ${active ? "bg-[rgba(0,245,138,0.08)] text-[var(--mint)]" : "text-secondary"}`}
                    >
              ...
              <div className="col-span-12 md:col-span-10 grid grid-cols-12 gap-3">
                ...
                <div className="col-span-12 lg:col-span-7 glass rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-5">
                    <div>
                      ...
                      <div className="text-lg mt-0.5">Agentic Confidential Send</div>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      >
                        Prepare Agent Transfer
                      </button>
                      <button className="rounded-xl px-4 py-3 text-sm glass-pill text-secondary">
                        Preview
                      </button>
                    </div>
                  </div>
                ...
                <div className="col-span-12 lg:col-span-5 glass rounded-2xl p-5">
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-2">
                    {["Deposit", "Withdraw", "Transfer"].map((a) => (
                      <button
                        key={a}
                        className="glass-pill rounded-lg py-2 text-[11px] font-mono uppercase tracking-[0.14em] text-secondary"
                      >
                        {a}
                      </button>
                    ))}
                  ...
                  <ol className="space-y-3">
                    {[
                      ...
                      <li key={i} className="flex items-center gap-3">
                        <span
                          className={`w-5 h-5 rounded-full grid place-items-center border ${done ? "bg-[var(--mint)] border-[var(--mint)]" : "border-[var(--border)]"}`}
                        >
                          {done ? (
                            <Check className="w-3 h-3 text-[var(--background)]" strokeWidth={3} />
                          ) : (
                            <ArrowDownToLine className="w-2.5 h-2.5 text-[var(--mint)] pulse-soft" />
                          )}
                        </span>
                        <span
                          className={`text-[13px] ${done ? "text-secondary" : "text-foreground"}`}
                        >
                          {label}
                        </span>
                      </li>
                ...
                <div className="col-span-12 lg:col-span-7 glass rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button className="font-mono text-[10px] uppercase tracking-[0.18em] text-secondary flex items-center gap-1">
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div
                        key={i}
                        className="grid grid-cols-12 items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-[var(--elevated)]/50"
                      >
                        <span className="col-span-3 font-mono text-[11px] uppercase tracking-[0.14em] text-secondary">
                          {row[0]}
                        </span>
                        <span className="col-span-4 font-mono text-[12px] text-foreground">
                          {row[1]}
                        </span>
                        <span className="col-span-3 font-mono text-[12px] text-muted">
                          {row[2]}
                        </span>
                        <span className="col-span-2 text-right font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--mint)]">
                          ● {row[3]}
                        </span>
                      </div>
                    ))}
  ...
  return (
    <div className="glass rounded-lg px-3.5 py-2.5">
      ...
      <div className="flex items-center justify-between">
        <span
          className={`text-sm font-mono ${mint ? "text-[var(--mint)] tracking-[0.2em]" : muted ? "text-muted" : "text-foreground"}`}
        >
          {value}
        </span>
