...
const cases = [
  ["Private payroll", "Pay contributors without publishing every compensation detail."],
  ["Treasury operations", "Move funds with less operational leakage."],
  ["OTC settlement", "Coordinate sensitive transfers with reduced public exposure."],
  ...
  return (
    <section id="use-cases" className="relative py-32 md:py-40 border-t border-[var(--border)]">
      <div className="max-w-[1280px] mx-auto px-6 md:px-10">
        <SectionHeader
          number="06" eyebrow="Use Cases"
          align="split"
          title={<>Built for serious<br />on-chain movement.</>}
          body="Wherever capital moves and exposure becomes a liability — payroll, treasury, OTC, DAO operations — Novault becomes the default settlement surface."
        />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-[var(--border)] glass rounded-2xl overflow-hidden">
          {cases.map(([t, b], i) => (
            <Reveal key={t} delay={i * 0.05}>
              ...
              >
                <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted">0{i + 1} / use</div>
                <div>
                  <div className="text-xl md:text-2xl mt-12 mb-3 group-hover:text-[var(--mint)] transition">{t}</div>
