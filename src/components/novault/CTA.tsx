...
export function CTA() {
  return (
    <section id="cta" className="relative pt-32 md:pt-44 pb-24 overflow-hidden border-t border-[var(--border)]">
      ...
      <div className="relative max-w-[1280px] mx-auto px-6 md:px-10 text-center">
        <Reveal>
          <div className="kicker mb-6">/// 08 — Enter Novault</div>
        </Reveal>
        <Reveal delay={0.1}>
          <h2 className="display text-4xl md:text-7xl text-balance max-w-4xl mx-auto">
            Move value without<br />exposing everything.
          </h2>
        </Reveal>
        <Reveal delay={0.2}>
          <p className="mt-8 text-secondary max-w-xl mx-auto leading-relaxed">
            Novault turns Solana transfers into private, verifiable, production-grade payment flows.
          </p>
        </Reveal>
        <Reveal delay={0.3}>
          <div className="mt-10 flex items-center justify-center gap-3 flex-wrap">
            <a href="#" className="glass-pill rounded-full pl-6 pr-2 py-2.5 inline-flex items-center gap-3 group" style={{ background: "rgba(0,245,138,0.10)", borderColor: "rgba(0,245,138,0.35)" }}>
              <span className="text-[var(--mint)]">Enter Novault</span>
            ...
            <a href="#foundation" className="glass-pill rounded-full px-6 py-3 text-secondary">Read the Architecture</a>
          </div>
        </Reveal>
        ...
        <div className="relative mt-20 select-none pointer-events-none">
          <h2
            ...
            style={{
              backgroundImage: "linear-gradient(180deg, rgba(243,255,248,0.22) 0%, rgba(0,245,138,0.10) 50%, transparent 100%)",
